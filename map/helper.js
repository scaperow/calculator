...
var selectionAdornmentTemplate =
  $(go.Adornment, "Auto", {
    ...
    $(go.Shape, 'RoundedRectangle',
      { fill: null, stroke: "#2196F3cc", strokeWidth: 1 },
...
function makeTwowayBinding(isRoot, ...names) {
  return _.map(names, (name) => {
    ...
    switch (name) {
      ...
      case 'smallWidth':
        binding = new go.Binding('width', 'smallWidth')
        break;

      case 'smallHeight':
        binding = new go.Binding('height', 'smallHeight')
        break;

      case 'location':
        binding = new go.Binding('loc', 'location')
        break;

      default:
        binding = new go.Binding(name, name).makeTwoWay()
    }
...
function makeOnewayBinding(isRoot, ...names) {
  return _.map(names, (name) => {
    var binding = null

    switch (name) {
      case 'fontColor':
        binding = new go.Binding('stroke', 'fontColor')
        break;

      case 'radius':
        binding = new go.Binding('parameter1', 'radius')
        break;
      ...
      default:
        binding = new go.Binding(name, name)
    }
...
function createTextBlock({ fontColor = '#333',
  ...
  return $(go.TextBlock, properties,
    ...
    ...makeTwowayBinding(false, 'text'),
    ...makeOnewayBinding(false, 'text', 'isUnderline', 'isStrikethrough'),
    ...props
  )
}
...
function layoutRoot(root, diagram) {
...
function layoutFamily(object) {
  ...
  var parentNode = object.diagram.findNodeForKey(parent)
  if (parentNode) {
...
function createPalatte(element, layout = 'Auto') {
  return $(go.Node, layout, ...elements)
}
...
function createIconTemplate({ fontSize } = { fontSize: 32 }) {
  return $(go.Panel, 'Auto',
    ...
    $(go.TextBlock, {
      ...
      editable: false,
      isMultiline: false
    }, ...makeTwowayBinding(false, 'stroke'),
...
function makePorts(portion = ['L', 'R', 'T', 'B']) {
  var ports = [{
    mouseLeave(e, el) {
  ...
  if (portion instanceof Boolean && portion === true) {
    ...
    ports.push(new go.Binding("itemArray", "ports"))

  } else if (portion instanceof Array) {
    ports = [...ports, ...portion.map(port => {
      if (port instanceof go.Shape) {
        return port
      } else {
        var align, spotSide, id = null

        switch (port) {
          case 'T':
            align = go.Spot.Top
            spotSide = go.Spot.TopSide
            break;
          case 'B':
            align = go.Spot.Bottom
            spotSide = go.Spot.BottomSide
            break;
          case 'R':
            align = go.Spot.Right
            spotSide = go.Spot.RightSide
            break;
          case 'L':
            align = go.Spot.Left
            spotSide = go.Spot.LeftSide
            break;

          default:
            return createPort(port.id, null, port.spot)
        }

        if (align && spotSide) {
          return createPort(port, align, spotSide)
        }
      }
    })]
  }
...
function createPort(name, alignment = null, spot, output = true, input = true) {
  return $(go.Shape,
    {
      name: 'PORT',
      ...
      stroke: 'transparent',
      strokeWidth: 1,
      width: 8,  // if not stretching horizont0,
      height: 8,  // if not stretching vertic0,
      alignment: alignment,  // align the port 0,
